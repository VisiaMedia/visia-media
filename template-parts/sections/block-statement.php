<?php if($statement = get_sub_field('statement')) {
	...
	<div class="statement js-statement" data-st-count="<?php $scrollTriggerCount--; echo $scrollTriggerCount; ?>"<?php echo $title ? ' aria-labelledby="'.esc_attr($titleID).'"' : ''; ?>>
		<div class="statement__content js-section-reveal">
			<div class="css-max-text-width">
                <?php if($title) {
                    echo '<span id="'.esc_attr($titleID).'" class="statement__content__title css-title--small-size css-title js-statement-title">'.$title.'</span>';
                } ?>
