<?php if($content = get_sub_field('content')) {
    ...
    <section class="textblock js-textblock"<?php echo $title ? ' aria-labelledby="'.esc_attr($titleID).'"' : ''; ?>>
        <div class="css-max-text-width">
            <div class="<?php echo(($format = get_sub_field('format')) ? 'textblock__content--'.$format.' ' : ''); ?>textblock__content js-section-reveal">
                <?php if($title) {
                    echo '<h1 id="'.esc_attr($titleID).'" class="textblock__content__title css-title--normal-size css-title js-textblock-title">'.do_shortcode($title).'</h1>';
                } ?>
